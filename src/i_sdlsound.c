...
static boolean (*ExpandSoundData)(sfxinfo_t *sfxinfo, byte *data,
                                  int samplerate, int bits, int length) = NULL;
...
static boolean ExpandSoundData_SRC(sfxinfo_t *sfxinfo, byte *data,
                                   int samplerate, int bits, int length)
{
    SRC_DATA src_data;
    float *data_in;
    ...
    src_data.output_frames =
        src_data.src_ratio * samplecount + (mixer_freq / 4);
    ...
    if (bits == 16)
    {
        for (i = 0; i < samplecount; ++i)
        {
            ...
            data_in[i] =
                (int16_t) (data[i * 2] | (data[i * 2 + 1] << 8)) / 32767.0;
        }
    }
    else
    {
                                   ...
                                   int samplerate, int bits, int length)
{
    ...
    expanded_length =
        (uint32_t) ((((uint64_t) samplecount) * mixer_freq) / samplerate);
    ...
    if (samplerate <= mixer_freq && ConvertibleRatio(samplerate, mixer_freq) &&
        SDL_BuildAudioCVT(&convertor, bits == 16 ? AUDIO_S16 : AUDIO_U8, 1,
                          samplerate, mixer_format, mixer_channels, mixer_freq))
    {
