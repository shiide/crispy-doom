...
void R_DrawMaskedColumn(column_t *column)
{
    ...
    for (; column->topdelta != 0xff;)
    {
        ...
        dc_yl = (int) ((topscreen + FRACUNIT - 1) >>
                       FRACBITS);                       // [crispy] WiggleFix
...
void R_DrawVisSprite(vissprite_t *vis, int x1, int x2)
{
    ...
    else if (crispy->translucency && vis->mobjflags & MF_TRANSLUCENT)
    {
        if (!(vis->mobjflags & (MF_NOGRAVITY | MF_COUNTITEM)) ||
            (vis->mobjflags & MF_NOGRAVITY &&
             crispy->translucency & TRANSLUCENCY_MISSILE) ||
            (vis->mobjflags & MF_COUNTITEM &&
...
void R_ProjectSprite(mobj_t *thing)
{
    ...
    if (sprframe->rotate)
    {
        ...
        if (sprframe->rotate == -1)
        {
            ...
            {
                const unsigned rot2 =
                    (ang - interpangle + (unsigned) (ANG45 / 4) * 17);
    ...
    if (crispy->flipcorpses && (thing->flags & MF_FLIPPABLE) &&
        !(thing->flags & MF_SHOOTABLE) && (thing->health & 1))
    {
        flip = !flip;
    }
    ...
    if (interpz > viewz + FixedDiv(viewheight << FRACBITS, xscale) ||
        gzt < (int64_t) viewz -
                  FixedDiv((viewheight << FRACBITS) - viewheight, xscale))
    {
        return;
    }
    ...
    {
        vis->blendfunc =
            (thing->frame & FF_FULLBRIGHT) ? I_BlendAdd : I_BlendOverTranmap;
    }
#endif
}

extern void P_LineLaser(mobj_t *t1, angle_t angle, fixed_t distance,
                        fixed_t slope);
...
byte *R_LaserspotColor(void)
{
    if (crispy->crosshairtarget)
    {
        ...
        {
            P_LineLaser(viewplayer->mo, viewangle, 16 * 64 * FRACUNIT,
                        PLAYER_SLOPE(viewplayer));
        }
        if (linetarget)
        {
            return cr[CR_GRAY];
...
static void R_DrawLSprite(void)
{
    ...
    vis->colormap[0] = vis->colormap[1] =
        fixedcolormap ? fixedcolormap
                      : colormaps; // [crispy] always full brightness
    ...
    if (vis->x1 < 0 || vis->x1 >= viewwidth || vis->x2 < 0 ||
        vis->x2 >= viewwidth)
        return;
...
void R_AddSprites(sector_t *sec)
{
    ...
    lightnum = (sec->rlightlevel >> LIGHTSEGSHIFT) +
...
void R_DrawPSprite(
    pspdef_t *psp,
    psprnum_t psprnum) // [crispy] differentiate gun from flash sprites
{
    fixed_t tx;
    int x1;
    int x2;
    ...
    // [crispy] free look
    vis->texturemid +=
        FixedMul(((centery - viewheight / 2) << FRACBITS), pspriteiscale) >>
        detailshift;
...
void R_DrawPlayerSprites(void)
{
    ...
    // get light level
    lightnum = (viewplayer->mo->subsector->sector->rlightlevel >>
                LIGHTSEGSHIFT) // [crispy] A11Y
