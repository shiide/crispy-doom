...
seeyou:
    if (actor->info->seesound)
    {
        ...
        if (actor->type == MT_SPIDER || actor->type == MT_CYBORG)
        {
            ...
            crispy->soundfull ? S_StartSoundOnce(NULL, sound)
            ...
            for (by = yl; by <= yh; by++)
            {
                ...
                if (!P_BlockThingsIterator(bx, by, PIT_VileCheck))
                {
                    ...
                    if (corpsehit->height == 0 && corpsehit->radius == 0)
                    {
                        ...
                        fprintf(stderr,
                                "A_VileChase: Resurrected ghost monster (%d) "
                                "at (%d/%d)!\n",
                                corpsehit->type, corpsehit->x >> FRACBITS,
...
void A_CloseShotgun2(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    A_ReFire(
        NULL, player,
        psp); // [crispy] let pspr action pointers get called from mobj states
}
         ...
         thinker = thinker->next)
    {
        ...
        if (m->type == MT_BOSSTARGET)
        {
            ...
            if (numbraintargets == maxbraintargets)
            {
                ...
                braintargets = I_Realloc(
                    braintargets, maxbraintargets * sizeof(*braintargets));
...
{
    // [crispy] prevent from adding up volume
    crispy->soundfull ? S_StartSoundOnce(NULL, sfx_bospn)
