...
void P_LoadSegs(int lump)
{
    ...
    {
        li->v1 = &vertexes[(unsigned short) SHORT(
            ml->v1)]; // [crispy] extended nodes
        li->v2 = &vertexes[(unsigned short) SHORT(
            ml->v2)]; // [crispy] extended nodes

        li->angle = (SHORT(ml->angle)) << FRACBITS;
        //	li->offset = (SHORT(ml->offset))<<FRACBITS; // [crispy] recalculated below
        linedef =
            (unsigned short) SHORT(ml->linedef); // [crispy] extended nodes
        ldef = &lines[linedef];
        li->linedef = ldef;
        side = SHORT(ml->side);
        ...
        if (ldef->flags & ML_TWOSIDED)
        {
            ...
            if (sidenum < 0 || sidenum >= numsides)
            {
                ...
                if (li->sidedef->midtexture)
                {
                    li->backsector = 0;
                    fprintf(stderr,
                            "P_LoadSegs: Linedef %d has two-sided flag set, "
...
void P_SegLengths(boolean contrast_only)
{
    ...
    for (i = 0; i < numsegs; i++)
    {
        ...
        {
            li->length =
                (uint32_t) (sqrt((double) dx * dx + (double) dy * dy) / 2);
...
void P_LoadSubsectors(int lump)
{
    ...
    {
        ss->numlines =
            (unsigned short) SHORT(ms->numsegs); // [crispy] extended nodes
        ss->firstline =
            (unsigned short) SHORT(ms->firstseg); // [crispy] extended nodes
    }
...
void P_LoadNodes(int lump)
{
    ...
    for (i = 0; i < numnodes; i++, no++, mn++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            no->children[j] = (unsigned short) SHORT(
                mn->children[j]); // [crispy] extended nodes
...
void P_LoadLineDefs(int lump)
{
    ...
    {
        ld->flags =
            (unsigned short) SHORT(mld->flags); // [crispy] extended nodes
        ld->special = SHORT(mld->special);
        // [crispy] warn about unknown linedef types
        if ((unsigned short) ld->special > 141 && ld->special != 271 &&
            ld->special != 272)
        {
        ...
        if (ld->special && !ld->tag)
        {
            switch (ld->special)
            {
                ...
                default:
                    fprintf(stderr,
                            "P_LoadLineDefs: Special linedef %d without tag.\n",
                            i);
                    warn2++;
                    break;
            }
        }
        v1 = ld->v1 = &vertexes[(unsigned short) SHORT(
            mld->v1)]; // [crispy] extended nodes
        v2 = ld->v2 = &vertexes[(unsigned short) SHORT(
            mld->v2)]; // [crispy] extended nodes
        ld->dx = v2->x - v1->x;
        ld->dy = v2->y - v1->y;
        ...
        if (ld->sidenum[0] == NO_INDEX)
        {
            ld->sidenum[0] = 0;
            fprintf(stderr,
                    "P_LoadLineDefs: linedef %d without first sidedef!\n", i);
        }
...
boolean P_LoadBlockMap(int lump)
{
    ...
    if (M_CheckParm("-blockmap") || lump >= numlumps ||
        (lumplen = W_LumpLength(lump)) < 8 || (count = lumplen / 2) >= 0x10000)
    {
        return false;
    }
            ...
            do
            {
                ...
                if (!v->moved)
                {
                    ...
                    if (v != l->v1 && v != l->v2)
                    {
                        ...
                        v->r_x = (fixed_t) ((dx2 * v->x + dy2 * l->v1->x +
                                             dxy * (v->y - l->v1->y)) /
                                            s);
                        v->r_y = (fixed_t) ((dy2 * v->y + dx2 * l->v1->y +
                                             dxy * (v->x - l->v1->x)) /
                                            s);

                        // [crispy] wait a minute... moved more than 8 map units?
                        // maybe that's a linguortal then, back to the original coordinates
                        if (abs(v->r_x - v->x) > 8 * FRACUNIT ||
                            abs(v->r_y - v->y) > 8 * FRACUNIT)
                        {
                            v->r_x = v->x;
                            v->r_y = v->y;
                        }
...
const char *skilltable[] = {"Nothing", "Baby", "Easy",
...
*/
    ...
    // [crispy] better logging
    {
        ...
        fprintf(stderr, "P_SetupLevel: %s (%s) %s%s %d:%02d:%02d/%d:%02d:%02d ",
                ...
                ltime / 3600, (ltime % 3600) / 60, ltime % 60, ttime / 3600,
    ...
    crispy_validblockmap = P_LoadBlockMap(
        lumpnum + ML_BLOCKMAP); // [crispy] (re-)create BLOCKMAP if necessary
