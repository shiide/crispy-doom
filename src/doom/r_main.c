...
angle_t R_PointToAngleSlope(fixed_t x, fixed_t y,
                            int (*slope_div)(unsigned int num,
                                             unsigned int den))
{
    x -= viewx;
    y -= viewy;
...
void R_InitLightTables(void)
{
    ...
    for (i = 0; i < LIGHTLEVELS; i++)
    {
        ...
        startmap =
            ((LIGHTLEVELS - LIGHTBRIGHT - i) * 2) * NUMCOLORMAPS / LIGHTLEVELS;
...
void R_ExecuteSetViewSize(void)
{
    ...
    else
    {
        ...
        if (crispy->widescreen)
        {
            ...
            scaledviewwidth = viewheight * SCREENWIDTH /
                              (SCREENHEIGHT - (ST_HEIGHT << crispy->hires));
            // [crispy] make sure scaledviewwidth is an integer multiple of the bezel patch width
            scaledviewwidth = (scaledviewwidth + widescreen_edge_aligner) &
                              (int) ~widescreen_edge_aligner;
    ...
    for (i = 0; i < LIGHTLEVELS; i++)
    {
        ...
        {
            level = startmap - j * NONWIDEWIDTH /
                                   (viewwidth_nonwide << detailshift) / DISTMAP;
    ...
    flipviewwidth = flipscreenwidth +
                    (crispy->fliplevels ? (SCREENWIDTH - scaledviewwidth) : 0);
...
void R_SetupFrame(player_t *player)
{
        ...
        leveltime > oldleveltime)
    {
        ...
        {
            viewangle =
                (player->mo->angle + localview.angle - localview.ticangle +
    ...
    tempCentery = viewheight / 2 + (pitch * (1 << crispy->hires)) *
                                       (screenblocks < 11 ? screenblocks : 11) /
                                       10;
    ...
    {
        fixedcolormap =
            colormaps + player->fixedcolormap * (NUMCOLORMAPS / 32) *
                            256; // [crispy] smooth diminishing lighting
