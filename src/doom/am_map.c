...
#define YOURRANGE  0
#define WALLCOLORS                                                             \
    (crispy->extautomap ? REDS + 4 : REDS) // [crispy] slightly darker red
#define WALLRANGE    REDRANGE
#define TSWALLCOLORS GRAYS
#define TSWALLRANGE  GRAYSRANGE
#define FDWALLCOLORS                                                           \
    (crispy->extautomap ? BROWNS + 6 : BROWNS) // [crispy] darker brown
#define FDWALLRANGE BROWNRANGE
#define CDWALLCOLORS                                                           \
    (crispy->extautomap ? 163 : YELLOWS) // [crispy] golden yellow
...
static mpoint_t m_paninc,
    m_paninc2; // how far the window pans each tic (map coords)
...
void AM_LevelInit(boolean reinit)
{
    ...
    AM_drawFline =
        crispy->smoothmap ? AM_drawFline_Smooth : AM_drawFline_Vanilla;
    ...
    else
    {
        ...
        a = FixedDiv(f_w,
                     (max_w >> MAPBITS < 2048) ? 2 * (max_w >> MAPBITS) : 4096);
        b = FixedDiv(f_h,
                     (max_h >> MAPBITS < 2048) ? 2 * (max_h >> MAPBITS) : 4096);
...
#undef REINDEX
            ...
            {
                color_shades[color * NUMSHADES + shade] =
                    colormaps[shade_index[shade]];
            }
        }
...
static void AM_CycleMobj(mobj_t **last_mobj, int *last_count, int flags,
                         int alive)
{
...
boolean AM_Responder(event_t *ev)
{
    ...
    else if (ev->type == ev_mouse && !crispy->automapoverlay && !menuactive &&
             !inhelpscreens)
    {
        ...
        else if (!followplayer && (ev->data2 || ev->data3))
        {
            ...
            const int flip_x = (ev->data2 * (mouseSensitivity_x2 + 5) /
                                (160 >> crispy->hires));
            m_paninc2.x = crispy->fliplevels ? -FTOM(flip_x) : FTOM(flip_x);
            m_paninc2.y = FTOM(ev->data3 * (mouseSensitivity_x2 + 5) /
                               (160 >> crispy->hires));
            rc = true;
        }
    }
...
#define DOOUTCODE(oc, mx, my)                                                  \
    ...
    while (outcode1 | outcode2)
    {
        ...
        else if (outside & BOTTOM)
        {
            ...
            dx = fl->b.x - fl->a.x;
            tmp.x = fl->a.x +
                    (fixed_t) (((int64_t) dx * (fl->a.y - (f_y + f_h))) / dy);
            tmp.y = f_h - 1;
        }
        ...
        if (cheating || (lines[i].flags & ML_MAPPED))
        {
                ...
                continue;
            {
                    ...
                    (amd = AM_DoorColor(lines[i].special)) > no_key)
                {
                    switch (amd)
                    {
                        case blue_key:
                            AM_drawMline(
                                &l, ((leveltime & 16) ? BLUES : GRIDCOLORS));
                            continue;
                        case yellow_key:
                            AM_drawMline(&l, ((leveltime & 16) ? (YELLOWS - 2)
                                                               : GRIDCOLORS));
                            continue;
                        case red_key:
                            AM_drawMline(&l, ((leveltime & 16) ? (REDS - 2)
                                                               : GRIDCOLORS));
                            continue;
                        default:
            ...
            else
            {
                ...
                if (lines[i].special == 39 ||
                    (crispy->extautomap && !(lines[i].flags & ML_SECRET) &&
                     lines[i].special == 97))
                { // teleporters
                    AM_drawMline(&l, crispy->extautomap
                                         ? (GREENS + GREENRANGE / 2)
...
static void AM_rotatePoint(mpoint_t *pt)
{
    ...
    tmpx =
        (int64_t) FixedMul(pt->x, finecosine[smoothangle >> ANGLETOFINESHIFT]) -
        (int64_t) FixedMul(pt->y, finesine[smoothangle >> ANGLETOFINESHIFT]) +
        mapcenter.x;

    pt->y =
        (int64_t) FixedMul(pt->x, finesine[smoothangle >> ANGLETOFINESHIFT]) +
...
void AM_drawPlayers(void)
{
    ...
    if (!netgame)
    {
        ...
        const angle_t smoothangle =
            crispy->automaprotate ? plr->mo->angle : viewangle;
    ...
    for (i = 0; i < MAXPLAYERS; i++)
    {
        ...
        AM_drawLineCharacter(player_arrow, arrlen(player_arrow), 0, theirangle,
                             color, pt.x, pt.y);
    }
}
            ...
            if (crispy->extautomap)
            {
                ...
                if (key > no_key)
                {
                    AM_drawLineCharacter(
                        cross_mark, arrlen(cross_mark), 16 << MAPBITS, t->angle,
                        (key == red_key)      ? REDS
