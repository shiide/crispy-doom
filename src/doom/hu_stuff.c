...
#define HU_COORDX                                                              \
    ((ORIGWIDTH - 8 * hu_font['A' - HU_FONTSTART]->width) + WIDESCREENDELTA)
...
const char *mapnames[] = // DOOM shareware/registered/retail (Ultimate) names.
    {
        ...
        // [crispy] Sigil
        HUSTR_E5M1, HUSTR_E5M2, HUSTR_E5M3, HUSTR_E5M4, HUSTR_E5M5, HUSTR_E5M6,
        HUSTR_E5M7, HUSTR_E5M8, HUSTR_E5M9,

        // [crispy] Sigil II
        HUSTR_E6M1, HUSTR_E6M2, HUSTR_E6M3, HUSTR_E6M4, HUSTR_E6M5, HUSTR_E6M6,
        HUSTR_E6M7, HUSTR_E6M8, HUSTR_E6M9,
...
const char *mapnames_commercial[] = {
    ...
    "", "", "", NHUSTR_1, NHUSTR_2, NHUSTR_3, NHUSTR_4, NHUSTR_5, NHUSTR_6,
    NHUSTR_7, NHUSTR_8, NHUSTR_9,

    MHUSTR_1, MHUSTR_2, MHUSTR_3, MHUSTR_4, MHUSTR_5, MHUSTR_6, MHUSTR_7,
    MHUSTR_8, MHUSTR_9, MHUSTR_10, MHUSTR_11, MHUSTR_12, MHUSTR_13, MHUSTR_14,
    MHUSTR_15, MHUSTR_16, MHUSTR_17, MHUSTR_18, MHUSTR_19, MHUSTR_20,
    MHUSTR_21};
...
static void CrispyReplaceColor(const char *str, const int cr, const char *col)
{
    ...
    M_snprintf(col_replace, sizeof(col_replace), "%s%s%s", crstr[cr], col,
               crstr[CR_NONE]);
...
void HU_Start(void)
{
    ...
    HUlib_initSText(&w_secret, 88, 86, HU_MSGHEIGHT, hu_font, HU_FONTSTART,
                    &secret_on);
    ...
    HUlib_initTextLine(&w_map, HU_TITLEX,
                       HU_TITLEY - SHORT(hu_font[0]->height + 1), hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_kills, HU_TITLEX, HU_MSGY + 1 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_items, HU_TITLEX, HU_MSGY + 2 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_scrts, HU_TITLEX, HU_MSGY + 3 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_ltime, HU_TITLEX, HU_MSGY + 4 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_coordx, HU_COORDX, HU_MSGY + 1 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_coordy, HU_COORDX, HU_MSGY + 2 * 8, hu_font,
                       HU_FONTSTART);

    HUlib_initTextLine(&w_coorda, HU_COORDX, HU_MSGY + 3 * 8, hu_font,
                       HU_FONTSTART);
        ...
        case pack_master:
            if (gamemap <= 21)
            {
                if (D_CheckMasterlevelKex())
                    s = mapnames_commercial[(kex_masterlevels[gamemap - 1] +
                                             105 + 3) -
                                            1];
                else
                    s = HU_TITLEM;
          ...
          !(crispy->havemaster && gamemission == pack_master))))
    {
        ...
        ptr = M_StringJoin(crstr[CR_GOLD], W_WadNameForLump(maplumpinfo), ": ",
                           crstr[CR_GRAY], maplumpinfo->name, NULL);
        m = ptr;
...
static void HU_DrawCrosshair(void)
{
    ...
    if (weaponinfo[plr->readyweapon].ammo == am_noammo ||
        ...
        (automapactive && !crispy->automapoverlay) || menuactive || paused ||
        secret_on)
        return;
...
void HU_Drawer(void)
{
    ...
    {
        ST_DrawDemoTimer(crispy->demotimerdir ? (deftotaldemotics - defdemotics)
                                              : defdemotics);
    }
...
static void Crispy_Statsline_Ratio(char *str, int str_size, const char *prefix,
                                   int count, int total, int extra)
{
    if (extra)
        M_snprintf(str, str_size, "%s%s%s%d/%d+%d ", cr_stat, prefix,
                   crstr[CR_GRAY], count, total, extra);
    else
        M_snprintf(str, str_size, "%s%s%s%d/%d ", cr_stat, prefix,
                   crstr[CR_GRAY], count, total);
}

static void Crispy_Statsline_Remaining(char *str, int str_size,
                                       const char *prefix, int count, int total,
                                       int extra)
{
...
static void Crispy_Statsline_Percent(char *str, int str_size,
                                     const char *prefix, int count, int total,
                                     int extra)
{
    M_snprintf(str, str_size, "%s%s%s%d%% ", cr_stat, prefix, crstr[CR_GRAY],
...
static void Crispy_Statsline_Boolean(char *str, int str_size,
                                     const char *prefix, int count, int total,
                                     int extra)
{
...
typedef void (*crispy_statsline_func_t)(char *str, int str_size,
...
void HU_Ticker(void)
{
    ...
    if (showMessages || message_dontfuckwithme)
    {
        ...
        if (plr->centermessage)
        {
            ...
            w_secret.l[0].x =
                ORIGWIDTH / 2 - M_StringWidth(plr->centermessage) / 2;
    ...
    if (netgame)
    {
        ...
        if (crispy->automapstats != WIDGETS_STBAR)
        {
            ...
            w_ltime.y =
                HU_MSGY + 4 * 8 + (crispy->automapstats ? chat_line : 0);
    ...
    if (crispy->automapstats == WIDGETS_STBAR &&
        (!automapactive || w_title.y != HU_TITLEY))
    {
        crispy_statsline_func_t crispy_statsline =
            crispy_statslines[crispy->statsformat];

        w_kills.x = -ST_WIDESCREENDELTA;

        w_kills.y = HU_TITLEY;

        crispy_statsline(str, sizeof(str), "K ", plr->killcount, totalkills,
                         extrakills);
        ...
        crispy_statsline(str, sizeof(str), "S ", plr->secretcount, totalsecret,
                         0);
        s = str;
        while (*s)
            HUlib_addCharToTextLine(&w_kills, *(s++));
             ...
             (automapactive && crispy->automapstats == WIDGETS_AUTOMAP))
    {
        ...
        w_kills.x =
            HU_TITLEX; // to handle switching from Status bar to Always and Automap kills line options

        crispy_statsline(str, sizeof(str), kills, plr->killcount, totalkills,
                         extrakills);
        HUlib_clearTextLine(&w_kills);
        s = str;
        while (*s)
        ...
        crispy_statsline(str, sizeof(str), "I\t", plr->itemcount, totalitems,
                         0);
    ...
    if (crispy->btusetimer && plr->btuse_tics)
    {
        ...
        M_snprintf(str, sizeof(str), "%sU\t%02i:%05.02f", crstr[CR_GRAY], mins,
                   secs);
        HUlib_clearTextLine(&w_ltime);
        s = str;
        while (*s)
    ...
    if (plr->powers[pw_showfps])
    {
        M_snprintf(str, sizeof(str), "%s%-4d %sFPS", crstr[CR_GRAY],
                   crispy->fps, cr_stat2);
