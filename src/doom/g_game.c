...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic)
{
    ...
    if (gamekeydown[key_up] ||
        gamekeydown[key_alt_up]) // [crispy] add key_alt_*
    {
        // fprintf(stderr, "up\n");
    ...
    if (crispy->freelook)
    {
        ...
        else if (joylook && joystick_look_sensitivity)
        {
            if (use_analog)
            {
...
void G_DoLoadLevel(void)
{
    ...
    if (crispy->havenerve || crispy->havemaster)
    {
        ...
        else if (crispy->havenerve && gameepisode == 2)
        {
        ...
        (gameversion == exe_final2 || gameversion == exe_chex || true))
    {
        ...
        // masterlevel skies
                 ...
                 (gameepisode == 3 || gamemission == pack_master))
        {
...
void G_PrepTiccmd(void)
{
    ...
    if (mousex && !strafe)
    {
        ...
        localview.angle = crispy->fliplevels ? -(basecmd.angleturn << 16)
                                             : (basecmd.angleturn << 16);
        mousex = 0;
    }
    ...
    if (mousey && crispy->mouselook)
    {
        const double vert = CalcMouseVert(mousey);
        basecmd.lookdir +=
            mouse_y_invert ? CarryPitch(-vert) : CarryPitch(vert);
        mousey = 0;
    }
}
...
void G_PlayerFinishLevel(int player)
{
    ...
    p->lookdir = p->oldlookdir = p->centering = p->jumpTics = p->recoilpitch =
        p->oldrecoilpitch = p->btuse = p->btuse_tics = 0;
}
...
static void G_FormatLevelStatTime(char *str, int tics)
{
    ...
    if (exitHours)
    {
        M_snprintf(str, TIMESTRSIZE, "%d:%02d:%05.2f", exitHours, exitMinutes,
                   exitSeconds);
    }
    else
    {
...
static void G_WriteLevelStat(void)
{
    ...
    {
        M_snprintf(levelString, sizeof(levelString), "E%dM%d", gameepisode,
                   gamemap);
    }
    ...
    fprintf(fstream, "%s%s - %s (%s)  K: %d/%d  I: %d/%d  S: %d/%d\n",
            levelString, (secretexit ? "s" : ""), levelTimeString,
            totalTimeString, playerKills, totalkills, playerItems, totalitems,
            playerSecrets, totalsecret);
...
*/
    ...
    else
    {
        ...
        else if (gamemap == 9)
        {
            ...
            wminfo.next =
                1; // [crispy] returning from secret level E1M10 "Sewers"
        else
             ...
             gameepisode == 6)
    {
        ...
        else if (gameversion == exe_chex && gameepisode == 1 && gamemap < 6)
        {
...
void G_DoSaveGame(void)
{
    ...
    // [crispy] some logging when saving
    {
        ...
        fprintf(stderr,
                "G_DoSaveGame: Episode %d, Map %d, %s, Time %d:%02d:%02d, "
                "Total %d:%02d:%02d.\n",
                gameepisode, gamemap,
                skilltable[BETWEEN(0, 5, (int) gameskill + 1)], ltime / 3600,
                (ltime % 3600) / 60, ltime % 60, ttime / 3600,
...
static void G_AddDemoFooter(void)
{
    ...
    filepos =
        WriteFileInfo("PORTNAME", strlen(PACKAGE_STRING), filepos, stream);
    filepos =
