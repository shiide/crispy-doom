...
static boolean LoadSigilWad(const char *iwaddir, boolean pwadtexture)
{
    ...
    const char *const sigil_wads[] = {"SIGIL_V1_23_REG.wad", "SIGIL_V1_23.wad",
                                      "SIGIL_v1_21.wad", "SIGIL_v1_2.wad",
                                      "SIGIL.wad"};

    static const lump_rename_t sigil_lumps[] = {
        {"CREDIT", "SIGCREDI"},   {"HELP1", "SIGHELP1"},
        {"TITLEPIC", "SIGTITLE"}, {"DEHACKED", "SIG_DEH"},
        {"DEMO1", "SIGDEMO1"},    {"DEMO2", "SIGDEMO2"},
        {"DEMO3", "SIGDEMO3"},    {"DEMO4", "SIGDEMO4"},
        {"D_INTER", "D_SIGINT"},  {"D_INTRO", "D_SIGTIT"},
    };
    ...
    if (i != -1 &&
        !strncasecmp(W_WadNameForLump(lumpinfo[i]), "SIGIL_COMPAT", 12))
    {
        return false;
    }
    ...
    sigil_shreds =
        M_StringJoin(iwaddir, DIR_SEPARATOR_S, "SIGIL_SHREDS.WAD", NULL);

    // [crispy] load SIGIL.WAD
    for (i = 0; i < arrlen(sigil_wads); i++)
    {
        crispy->havesigil =
            M_StringJoin(iwaddir, DIR_SEPARATOR_S, sigil_wads[i], NULL);
    ...
    for (i = 0; i < arrlen(sigil_lumps); i++)
    {
        ...
        if (j != -1 &&
            !strncasecmp(W_WadNameForLump(lumpinfo[j]), "SIGIL_SHREDS", 12))
        {
    ...
    for (i = 0; i < arrlen(sigil_lumps); i++)
    {
        ...
        if (j != -1 &&
            !strcasecmp(W_WadNameForLump(lumpinfo[j]), sigil_basename))
        {
            memcpy(lumpinfo[j]->name, sigil_lumps[i].new_name, 8);
        }
    }
    ...
    if (!M_ParmExists("-noautoload"))
    {
        ...
        {
            W_AutoLoadWADsRename(autoload_dir, sigil_lumps,
                                 arrlen(sigil_lumps));
...
static boolean LoadSigil2Wad(const char *iwaddir, boolean pwadtexture)
{
    ...
    static const lump_rename_t sigil2_lumps[] = {
        {"CREDIT", "SG2CREDI"},   {"HELP1", "SG2HELP1"},
        {"TITLEPIC", "SG2TITLE"}, {"SIGILEND", "SGL2END"},
        {"DEHACKED", "SG2_DEH"},  {"DEMO1", "SG2DEMO1"},
        {"DEMO2", "SG2DEMO2"},    {"DEMO3", "SG2DEMO3"},
        {"DEMO4", "SG2DEMO4"},    {"D_INTER", "D_SG2INT"},
        {"D_INTRO", "D_SG2TIT"},
    };
    ...
    {
        crispy->havesigil2 =
            M_StringJoin(iwaddir, DIR_SEPARATOR_S, sigil2_wads[i], NULL);
    ...
    for (i = 0; i < arrlen(sigil2_lumps); i++)
    {
        ...
        if (j != -1 &&
            !strcasecmp(W_WadNameForLump(lumpinfo[j]), sigil2_basename))
        {
    ...
    if (!M_ParmExists("-noautoload"))
    {
        ...
        {
            W_AutoLoadWADsRename(autoload_dir, sigil2_lumps,
                                 arrlen(sigil2_lumps));
            DEH_AutoLoadPatches(autoload_dir);
            free(autoload_dir);
        }
...
static void CheckLoadNerve(void)
{
    ...
    static const lump_rename_t nerve_lumps[] = {
        {"TITLEPIC", "NERVEPIC"}, {"INTERPIC", "NERVEINT"},
        {"M_DOOM", "M_DOOM_N"},   {"DEMO1", "DEMO1N"},
        {"DEMO2", "DEMO2N"},      {"DEMO3", "DEMO3N"},
        {"RSKY1", "RSKY1N"},      {"RSKY2", "RSKY2N"},
        {"RSKY3", "RSKY3N"},
    };
    ...
    if (strrchr(iwadfile, DIR_SEPARATOR) != NULL)
    {
        ...
        crispy->havenerve =
            M_StringJoin(dir, DIR_SEPARATOR_S, "NERVE.WAD", NULL);
        free(dir);
    }
    else
    {
    ...
    for (i = 0; i < arrlen(nerve_lumps); i++)
    {
        ...
        if (j != -1 &&
            !strcasecmp(W_WadNameForLump(lumpinfo[j]), nerve_basename))
        {
    ...
    if (!M_ParmExists("-noautoload"))
    {
        ...
        {
            W_AutoLoadWADsRename(autoload_dir, nerve_lumps,
                                 arrlen(nerve_lumps));
            DEH_AutoLoadPatches(autoload_dir);
            free(autoload_dir);
        }
...
static const lump_rename_t master_lumps[] = {
    {"TITLEPIC", "MASTRPIC"}, {"INTERPIC", "MASTRINT"}, {"M_DOOM", "M_DOOM_M"},
    {"DEMO1", "DEMO1M"},      {"DEMO2", "DEMO2M"},      {"DEMO3", "DEMO3M"},
    {"RSKY1", "RSKY1M"},      {"RSKY2", "RSKY2M"},      {"RSKY3", "RSKY3M"},
};
...
static boolean CheckLoadMasterlevels(void)
{
    ...
    if (strrchr(iwadfile, DIR_SEPARATOR) != NULL)
    {
        ...
        dir = M_DirName(iwadfile);
        crispy->havemaster =
            M_StringJoin(dir, DIR_SEPARATOR_S, "MASTERLEVELS.WAD", NULL);
        free(dir);
    }
    else
    {
    ...
    for (i = 0; i < arrlen(master_lumps); i++)
    {
        ...
        if (j != -1 &&
            !strcasecmp(W_WadNameForLump(lumpinfo[j]), master_basename))
        {
    ...
    if (!M_ParmExists("-noautoload"))
    {
        ...
        {
            W_AutoLoadWADsRename(autoload_dir, master_lumps,
                                 arrlen(master_lumps));
            DEH_AutoLoadPatches(autoload_dir);
            free(autoload_dir);
        }
...
static boolean CheckMasterlevelsAvailable(void)
{
    ...
    for (i = 0; masterlevels_wads[i].wad_name; i++)
    {
        ...
        if (strrchr(iwadfile, DIR_SEPARATOR) != NULL)
        {
            havemaster = M_StringJoin(dir, DIR_SEPARATOR_S,
                                      masterlevels_wads[i].wad_name, NULL);
        }
        else
        {
        ...
        if (masterlevels_wads[i].wad_name)
        {
            ...
            if (masterlevels_wads[i].custom_sky)
            {
                for (j = 0; j < arrlen(sky_lumps); j++)
                {
                    ...
                    if (k != -1 && !strcasecmp(W_WadNameForLump(lumpinfo[k]),
                                               masterlevels_wads[i].wad_name))
                    {
                        lumpinfo[k]->name[0] = 'M';
                    }
                }
