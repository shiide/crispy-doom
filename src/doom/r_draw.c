...
#else
        *dest = I_BlendDark(
            dest[SCREENWIDTH * (fuzzoffset[fuzzpos] - FUZZOFF) / 2], 0xD3);
#endif
    }
}
...
void R_DrawFuzzColumnLow(void)
{
    ...
    if (cutoff)
    {
        ...
        *dest = I_BlendDark(
            dest[SCREENWIDTH * (fuzzoffset[fuzzpos] - FUZZOFF) / 2], 0xD3);
        *dest2 = I_BlendDark(
            dest2[SCREENWIDTH * (fuzzoffset[fuzzpos] - FUZZOFF) / 2], 0xD3);
#endif
    }
}
...
void R_DrawTLColumn(void)
{
    ...
    do
    {
        ...
        *dest =
            tranmap[(*dest << 8) + dc_colormap[dc_brightmap[source]][source]];
#else
...
void R_DrawTLColumnLow(void)
{
    ...
    do
    {
        ...
        *dest =
            tranmap[(*dest << 8) + dc_colormap[dc_brightmap[source]][source]];
        *dest2 =
            tranmap[(*dest2 << 8) + dc_colormap[dc_brightmap[source]][source]];
#else
        const pixel_t destrgb = dc_colormap[dc_brightmap[source]][source];
        *dest = blendfunc(*dest, destrgb);
...
void R_FillBackScreen(void)
{
    ...
    for (x = 0; x < (scaledviewwidth >> crispy->hires); x += 8)
        V_DrawPatch((viewwindowx >> crispy->hires) + x - WIDESCREENDELTA,
                    (viewwindowy >> crispy->hires) - 8, patch);
    patch = W_CacheLumpName(DEH_String("brdr_b"), PU_CACHE);

    for (x = 0; x < (scaledviewwidth >> crispy->hires); x += 8)
        V_DrawPatch((viewwindowx >> crispy->hires) + x - WIDESCREENDELTA,
                    (viewwindowy >> crispy->hires) +
                        (viewheight >> crispy->hires),
                    patch);
    patch = W_CacheLumpName(DEH_String("brdr_l"), PU_CACHE);

    for (y = 0; y < (viewheight >> crispy->hires); y += 8)
        V_DrawPatch((viewwindowx >> crispy->hires) - 8 - WIDESCREENDELTA,
                    (viewwindowy >> crispy->hires) + y, patch);
    patch = W_CacheLumpName(DEH_String("brdr_r"), PU_CACHE);

    for (y = 0; y < (viewheight >> crispy->hires); y += 8)
        V_DrawPatch((viewwindowx >> crispy->hires) +
                        (scaledviewwidth >> crispy->hires) - WIDESCREENDELTA,
                    (viewwindowy >> crispy->hires) + y, patch);
