...
void P_CalcHeight(player_t *player)
{
    ...
    bob = FixedMul(player->bob2 / 2,
                   finesine[angle]); // [crispy] variable player view bob
...
void P_MovePlayer(player_t *player)
{
    ...
    {
        player->lookdir =
            BETWEEN(-LOOKDIRMIN * MLOOKUNIT, LOOKDIRMAX * MLOOKUNIT,
...
void P_PlayerThink(player_t *player)
{
    ...
    if (cmd->buttons & BT_CHANGE)
    {
        ...
        if ((crispy->havessg) && newweapon == wp_shotgun &&
    ...
    if (player->powers[pw_invulnerability])
    {
        if (player->powers[pw_invulnerability] > 4 * 32 ||
            ...
            player->fixedcolormap =
                a11y_invul_colormap ? INVERSECOLORMAP : 1; // [crispy] A11Y
        else
            // [crispy] Visor effect when Invulnerability is fading out
            player->fixedcolormap =
                (player->powers[pw_infrared] && a11y_invul_colormap)
                    ? 1
                    : 0; // [crispy] A11Y
    }
