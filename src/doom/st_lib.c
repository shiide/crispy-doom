...
void STlib_drawNum(st_number_t *n, boolean refresh)
{
    ...
    if (screenblocks < CRISPY_HUD || (automapactive && !crispy->automapoverlay))
        V_CopyRect(x + WIDESCREENDELTA, n->y - ST_Y, st_backing_screen,
                   w * numdigits, h, x + WIDESCREENDELTA, n->y);
    ...
    if (*mi->on && (mi->oldinum != *mi->inum || refresh) && (*mi->inum != -1))
    {
        if (mi->oldinum != -1)
        {
            ...
            if (screenblocks < CRISPY_HUD ||
                ...
                V_CopyRect(x + WIDESCREENDELTA, y - ST_Y, st_backing_screen, w,
                           h, x + WIDESCREENDELTA, y);
        }
...
void STlib_updateBinIcon(st_binicon_t *bi, boolean refresh)
{
    ...
    if (*bi->on && (bi->oldval != *bi->val || refresh))
    {
        ...
        else if (screenblocks < CRISPY_HUD ||
                 (automapactive && !crispy->automapoverlay))
            V_CopyRect(x + WIDESCREENDELTA, y - ST_Y, st_backing_screen, w, h,
                       x + WIDESCREENDELTA, y);
