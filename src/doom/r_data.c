...
unsigned **
    texturecolumnofs2; // [crispy] column offsets for composited opaque textures
byte **
    texturecomposite; // [crispy] composited translucent mid-textures on 2S walls
...
lighttable_t
    *pal_color; // [crispy] array holding palette colors for true color mode
    ...
    for (i = 0; i < texture->width; i++)
    {
        ...
        //	if (collump[i] == -1) // process only multipatched columns
        {
            ...
            for (;;) // reconstruct the column by scanning transparency marks
            {
                ...
                col->length =
                    len; // killough 12/98: intentionally truncate length
...
void R_GenerateLookup(int texnum)
{
    ...
    byte *
        postcount; // killough 4/9/98: keep count of posts in addition to patches.
    texpatch_t *patch;
    patch_t *realpatch;
    int x;
    int x1;
...
void R_InitTextures(void)
{
    ...
    texturelumps =
        I_Realloc(texturelumps, maxtexturelumps * sizeof(*texturelumps));

    // [crispy] make sure the first available TEXTURE1/2 lumps
    // are always processed first
    texturelumps[numtexturelumps++].lumpnum =
        W_GetNumForName(DEH_String("TEXTURE1"));
    ...
    for (i = numlumps - 1; i >= 0; i--)
    {
        if (!strncasecmp(lumpinfo[i]->name, DEH_String("PNAMES"), 6))
        {
            ...
            {
                maxpnameslumps++;
                pnameslumps = I_Realloc(pnameslumps,
                                        maxpnameslumps * sizeof(*pnameslumps));
            }
        ...
        else if (!strncasecmp(lumpinfo[i]->name, DEH_String("TEXTURE"), 7))
        {
            ...
            {
                maxtexturelumps++;
                texturelumps = I_Realloc(
                    texturelumps, maxtexturelumps * sizeof(*texturelumps));
            }
    ...
    {
        texturelumps[i].maptex =
            W_CacheLumpNum(texturelumps[i].lumpnum, PU_STATIC);
    ...
    texturebrightmap =
        Z_Malloc(numtextures * sizeof(*texturebrightmap), PU_STATIC, 0);
    ...
    if (I_ConsoleStdout())
    {
        ...
        for (i = 0; i < temp3 + 9 + 1;
             i++) // [crispy] one more for R_InitTranMap()
#else
        for (i = 0; i < temp3 + 9; i++)
#endif
            printf(" ");
    ...
    for (i = 0; i < numtextures; i++, directory++)
    {
        ...
        for (j = 0; j < texture->patchcount; j++, mpatch++, patch++)
        {
            ...
            if (patch->patch == -1 || p >= nummappatches)
            {
                ...
                fprintf(stderr, "R_InitTextures: Missing patch in texture %s\n",
                        texturename);
                patch->patch =
                    W_CheckNumForName("WIPCNT"); // [crispy] dummy patch
            }
        }
        ...
        texturecolumnofs2[i] = Z_Malloc(
            texture->width * sizeof(**texturecolumnofs2), PU_STATIC, 0);
        ...
        tranmap = Z_Malloc(256 * 256, PU_STATIC, 0);
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                for (j = 0; j < 256; j++)
                {
                    ...
                    *tp++ = V_GetPaletteIndex(playpal, blend[r], blend[g],
                                              blend[b]);
                }
            }
        }
...
void R_InitColormaps(void)
{
    ...
    colormaps =
        I_Realloc(colormaps, (NUMCOLORMAPS + 1) * 256 * sizeof(lighttable_t));
    ...
    if (crispy->truecolor)
    {
        for (c = 0; c < NUMCOLORMAPS; c++)
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                r = gamma2table[crispy->gamma][playpal[3 * k + 0]] *
                        (1. - scale) +
                    gamma2table[crispy->gamma][0] * scale;
        ...
        for (i = 0; i < 256; i++)
        {
            const byte gray = 0xff - (byte) (0.299 * playpal[3 * i + 0] +
                                             0.587 * playpal[3 * i + 1] +
                                             0.114 * playpal[3 * i + 2]);
    ...
    else
    {
        for (c = 0; c <= NUMCOLORMAPS; c++)
        {
            for (i = 0; i < 256; i++)
            {
                r = gamma2table[crispy->gamma]
                               [playpal[3 * colormap[c * 256 + i] + 0]] &
                    ~3;
                g = gamma2table[crispy->gamma]
                               [playpal[3 * colormap[c * 256 + i] + 1]] &
                    ~3;
                b = gamma2table[crispy->gamma]
                               [playpal[3 * colormap[c * 256 + i] + 2]] &
                    ~3;
