...
static void saveg_read_mobj_t(mobj_t *str)
{
    ...
    if (pl > 0)
    {
        ...
        str->player->mo = str;
        str->player->so =
            Crispy_PlayerSO(pl - 1); // [crispy] weapon sound sources
    }
    else
    {
        str->player = NULL;
...
static void saveg_write_mobj_t(mobj_t *str)
{
    ...
    saveg_writep(
        (void *) (uintptr_t) P_ThinkerToIndex((thinker_t *) str->target));
    ...
    saveg_writep(
        (void *) (uintptr_t) P_ThinkerToIndex((thinker_t *) str->tracer));
}
...
static void saveg_read_strobe_t(strobe_t *str)
{
    ...
    if (!a11y_sector_lighting &&
        str->sector->rlightlevel <
            str->maxlight) // [crispy] Ensure maxlight among competing thinkers.
        str->sector->rlightlevel = str->maxlight;
}
