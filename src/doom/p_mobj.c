...
boolean P_SetMobjState(mobj_t *mobj, statenum_t state)
{
    ...
    do
    {
        ...
        if (st->action.acp3)
            st->action.acp3(
                mobj, NULL,
                NULL); // [crispy] let pspr action pointers get called from mobj states
...
// P_SpawnMobj
//
static mobj_t *P_SpawnMobjSafe(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type,
                               boolean safe)
{
    mobj_t *mobj;
    state_t *st;
    ...
    mobj->lastlook =
        safe ? Crispy_Random() % MAXPLAYERS : P_Random() % MAXPLAYERS;
    ...
    if (!info->actualheight)
    {
        ...
        else
        {
            ...
            info->actualheight = ((SHORT(patch->height) + 7) >> 3)
                                 << (FRACBITS + 3);
        }
    }
...
void P_SpawnMapThing(mapthing_t *mthing)
{
    ...
    {
        fprintf(stderr,
                "P_SpawnMapThing: Mapthing type %i without any skill tag at "
                "(%i, %i)\n",
