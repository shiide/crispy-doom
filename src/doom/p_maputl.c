...
static void check_intercept(void)
{
    ...
    {
        num_intercepts =
            num_intercepts ? num_intercepts * 2 : MAXINTERCEPTS_ORIGINAL;
        intercepts =
            I_Realloc(intercepts, sizeof(*intercepts) * num_intercepts);
...
boolean PIT_AddLineIntercepts(line_t *ld)
{
    ...
    if (intercept_p - intercepts == MAXINTERCEPTS_ORIGINAL + 1)
    {
        if (crispy->crosshair & CROSSHAIR_INTERCEPT)
            ...
            fprintf(stderr,
                    "PIT_AddLineIntercepts: Triggered INTERCEPTS overflow!\n");
    }
    intercept_p++;
