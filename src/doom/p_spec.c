...
void P_InitPicAnims(void)
{
    ...
    for (i = 0; animdefs[i].istexture != -1; i++)
    {
        ...
        lastanim->speed =
            from_lump ? LONG(animdefs[i].speed) : animdefs[i].speed;
...
fixed_t P_FindNextHighestFloor(sector_t *sec, int currentheight)
{
    ...
    if (sec->linecount > heightlist_size)
    {
        ...
        {
            heightlist_size =
                heightlist_size ? 2 * heightlist_size : MAX_ADJOINING_SECTORS;
        } while (sec->linecount > heightlist_size);
        heightlist =
            I_Realloc(heightlist, heightlist_size * sizeof(*heightlist));
    }
    ...
    switch (sector->special)
    {
        ...
        case 9:
                ...
                player == &players[consoleplayer])
            {
                ...
                M_snprintf(str_count, sizeof(str_count),
                           "Secret %d of %d revealed!", player->secretcount,
                           totalsecret);
                ...
                player->centermessage =
                    (crispy->secretmessage == SECRETMESSAGE_COUNT)
                        ? str_count
                        : HUSTR_SECRETFOUND;
                if (sfx_id != -1)
