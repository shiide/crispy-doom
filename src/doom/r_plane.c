...
void R_MapPlane(int y, int x1, int x2)
{
    ...
    dy = (abs(centery - y) << FRACBITS) +
         (y < centery ? -FRACUNIT : FRACUNIT) / 2;
...
visplane_t *R_CheckPlane(visplane_t *pl, int start, int stop)
{
    ...
    if (lastvisplane - visplanes == MAXVISPLANES &&
        false) // [crispy] remove VISPLANES limit
...
void R_DrawPlanes(void)
{
    ...
    for (pl = visplanes; pl < lastvisplane; pl++)
    {
        ...
        if (pl->picnum == skyflatnum || pl->picnum & PL_SKYFLAT)
        {
            ...
            dc_texheight =
                textureheight[texture] >> FRACBITS; // [crispy] Tutti-Frutti fix
        ...
        // regular flat
        lumpnum =
            firstflat + (swirling ? pl->picnum : flattranslation[pl->picnum]);
        // [crispy] add support for SMMU swirling flats
        ds_source = swirling ? R_DistortedFlat(lumpnum)
