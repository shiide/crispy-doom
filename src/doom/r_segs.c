...
static const struct
{
    int clamp;
    int heightbits;
} scale_values[8] = {{2048 * FRACUNIT, 12}, {1024 * FRACUNIT, 12},
...
void R_FixWiggle(sector_t *sector)
{
    ...
    int height =
        (sector->interpceilingheight - sector->interpfloorheight) >> FRACBITS;
    ...
    for (dc_x = x1; dc_x <= x2; dc_x++)
    {
        ...
        if (maskedtexturecol[dc_x] != INT_MAX) // [crispy] 32-bit integer math
        {
...

            {
                ...
                {
                    spryscale +=
                        rw_scalestep; // [crispy] MBF had this in the for-loop iterator
            ...
            // draw the texture
            col = (column_t *) ((byte *) R_GetColumnMasked(
...
void R_RenderSegLoop(void)
{
    ...
    for (; rw_x < rw_stopx; rw_x++)
    {
        ...
        yl = (int) ((topfrac + heightunit - 1) >>
                    heightbits); // [crispy] WiggleFix
        ...
        if (segtextured)
        {
            ...
            dc_colormap[0] = walllights[index];
            dc_colormap[1] =
                (!fixedcolormap && (crispy->brightmaps & BRIGHTMAPS_TEXTURES))
                    ? colormaps
                    : dc_colormap[0];
            dc_x = rw_x;
        ...
        if (midtexture)
        {
            ...
            dc_texheight = textureheight[midtexture] >>
                           FRACBITS; // [crispy] Tutti-Frutti fix
        ...
        else
        {
            ...
            if (toptexture)
            {
                ...
                if (mid >= yl)
                {
                    ...
                    dc_texheight = textureheight[toptexture] >>
                                   FRACBITS; // [crispy] Tutti-Frutti fix
            ...
            if (bottomtexture)
            {
                ...
                if (mid <= yh)
                {
                    ...
                    dc_texheight = textureheight[bottomtexture] >>
                                   FRACBITS; // [crispy] Tutti-Frutti fix
...
void R_StoreWallRange(int start, int stop)
{
    ...
    rw_normalangle =
        curline->r_angle + ANG90; // [crispy] use re-calculated angle
...
*/
    ...
    topfrac =
        ((int64_t) centeryfrac >> invhgtbits) -
