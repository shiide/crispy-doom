...
void P_InitSwitchList(void)
{
    ...
    for (i = 0; alphSwitchList[i].episode; i++)
    {
        const short alphSwitchList_episode =
            from_lump ? SHORT(alphSwitchList[i].episode)
    ...
    buttonlist =
        I_Realloc(NULL, sizeof(*buttonlist) * (maxbuttons = MAXBUTTONS));
...
void P_StartButton(line_t *line, bwhere_e w, int texture, int time)
{
        ...
        if (!buttonlist[i].btimer)
        {
            ...
            buttonlist[i].soundorg =
                crispy->soundfix
                    ? &line->soundorg
                    : &line->frontsector
                           ->soundorg; // [crispy] corrected sound source
            return;
        }
    }
    ...
    // [crispy] remove MAXBUTTONS limit
    {
        ...
        memset(buttonlist + maxbuttons / 2, 0,
               sizeof(*buttonlist) * maxbuttons / 2);
...
void P_ChangeSwitchTexture(line_t *line, int useAgain)
{
    ...
    if (playsound)
    {
        S_StartSound(crispy->soundfix ? &line->soundorg : buttonlist->soundorg,
                     sound);
    }
}
