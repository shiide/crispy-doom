    ...
    if (ld->special)
    {
        ...
        if (numspechit > MAXSPECIALCROSS_ORIGINAL)
        {
            ...
            if (numspechit == MAXSPECIALCROSS_ORIGINAL + 1)
                fprintf(stderr,
                        "PIT_CheckLine: Triggered SPECHITS overflow!\n");
            SpechitOverrun(ld);
        }
    }
    ...
    if (critical->overunder)
    {
        ...
        if (tmthing->player && thing->flags & MF_SHOOTABLE)
        {
            ...
            else if (tmthing->z + tmthing->height <= thing->z)
            {
                ...
                thing->floorz =
                    MAX(tmthing->z + tmthing->height, thing->floorz);
                return true;
            }
            ...
            {
                fixed_t newdist =
                    P_AproxDistance(thing->x - tmx, thing->y - tmy);
                fixed_t olddist = P_AproxDistance(thing->x - tmthing->x,
                                                  thing->y - tmthing->y);
        ...
        if (aimslope)
        {
            ...
            if ((side = li->sidenum[lineside]) != NO_INDEX)
            {
                     ...
                     sector->ceilingpic != skyflatnum))
                {
                    ...
                    frac =
                        FixedDiv(z - shootz, FixedMul(aimslope, attackrange));
                    x = trace.x + FixedMul(trace.dx, frac);
                    y = trace.y + FixedMul(trace.dy, frac);
                }
            }
                  ...
                  int damage)
{
    ...
    shootz =
        (damage == INT_MIN) ? viewz : t1->z + (t1->height >> 1) + 8 * FRACUNIT;
    attackrange = distance;
    aimslope = slope;

    P_PathTraverse(t1x, t1y, x2, y2, PT_ADDLINES | PT_ADDTHINGS,
                   PTR_ShootTraverse);
}
