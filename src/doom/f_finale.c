...
void F_StartFinale(void)
{
    ...
    if (logical_gamemission == pack_master && D_CheckMasterlevelKex() &&
        players[consoleplayer].didsecret)
    {
        finaletext = M2TEXT;
    }
...
static int F_SoundForState(int st)
{
    ...
    void *const nextaction =
        (void *) (&states[caststate->nextstate])->action.acv;
...
void F_CastTicker(void)
{
        ...
        castskip) // [crispy] skippable cast
    {
        ...
        if (mobjinfo[castorder[castnum].type].seesound)
            S_StartSound(NULL, F_RandomizeSound(
...
void F_CastDrawer(void)
{
    ...
    V_DrawPatchFullScreen(W_CacheLumpName(DEH_String("BOSSBACK"), PU_CACHE),
                          false);
...
void F_BunnyScroll(void)
{
    ...
    if (pillar_width > 0)
    {
        ...
        V_DrawFilledBox(SCREENWIDTH - pillar_width, 0, pillar_width,
                        SCREENHEIGHT, 0);
    }
    else
    {
    ...
    if (finalecount < 1180)
    {
        V_DrawPatch((ORIGWIDTH - 13 * 8) / 2, (ORIGHEIGHT - 8 * 8) / 2,
