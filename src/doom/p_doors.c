    ...
    switch (line->special)
    {
        ...
        case 133:
            if (!p->cards[it_bluecard] && !p->cards[it_blueskull])
            {
                ...
                S_StartSoundOptional(crispy->soundfix ? p->mo : NULL,
                                     sfx_locked, sfx_oof);
    ...
    switch (line->special)
    {
        ...
        case 32:
            ...
            if (!player->cards[it_bluecard] && !player->cards[it_blueskull])
            {
                ...
                // [NS] Locked door sound.
                S_StartSoundOptional(crispy->soundfix ? player->mo : NULL,
                                     sfx_locked, sfx_oof);
                // [crispy] blinking key or skull in the status bar
        ...
        switch (line->special)
        {
            ...
            case 117:
                if (door->direction == -1)
                {
                    ...
                    if (crispy->soundfix && door->thinker.function.acp1 ==
                                                (actionf_p1) T_VerticalDoor)
                        S_StartSound(&door->sector->soundorg,
                                     line->special == 117 ? sfx_bdopn
                                                          : sfx_doropn);
                }
                else
                {
