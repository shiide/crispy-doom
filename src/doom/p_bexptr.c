...
void A_RandomJump(mobj_t *mo, player_t *player, pspdef_t *psp)
{
    ...
    if (player && psp)
    {
        ...
        {
            extern void P_SetPsprite(player_t * player, int position,
                                     statenum_t stnum);
...
void A_FireOldBFG(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    do
    {
        ...
        angle_t an = mo->angle;
        angle_t an1 =
            ((P_Random(/* pr_bfg */) & 127) - 64) * (ANG90 / 768) + an;
        angle_t an2 =
            ((P_Random(/* pr_bfg */) & 127) - 64) * (ANG90 / 640) + ANG90;
        //    extern int autoaim;
                ...
                do
                {
                    ...
                    if (!linetarget)
                        slope = P_AimLineAttack(mo, an -= 2 << 26,
                                                16 * 64 * FRACUNIT); //, mask);
                    if (!linetarget)
                        slope = (critical->freeaim == FREEAIM_BOTH)
                                    ? PLAYER_SLOPE(player)
                                    : 0,
                        an = mo->angle;
        ...
        th->momx =
            FixedMul(th->info->speed, finecosine[an1 >> ANGLETOFINESHIFT]);
        th->momy = FixedMul(th->info->speed, finesine[an1 >> ANGLETOFINESHIFT]);
        th->momz =
            FixedMul(th->info->speed, finetangent[an2 >> ANGLETOFINESHIFT]);
